...
ObjectDetection::ObjectDetection(/* args */) : Node("lidar3d_clustering_node"), front_zone(Point{-1.2, 0.0}, Point{-1.2, 2.0}, Point{1.2, 2.0}, Point{1.2, 0.0}), front_zone_warning(Point{2.0, -1.0}, Point{2.0, 1.0}, Point{4.0, 1.0}, Point{4.0, -1.0}) 
{
    // Parameters
...
void ObjectDetection::publish_zone(const Zone& zone) {
    ...
    for (const auto& corner : zone.corners) {
        ...
        p.x = corner.x;
        p.y = corner.y;
        p.z = 0; // Assuming the zone is flat on the ground plane
        ros_points.push_back(p);
    }

    // Close the loop by adding the first point again at the end
    ros_points.push_back(ros_points.front());

    marker.points = ros_points;
